                        ...
                        <div>
                            @foreach ($product->attributevalues->unique('product_attribute_id') as $av)
                                ...
                                <div class="col-xs-2">
                                    ...
                                    <ul class="list-style inline-round" wire:model="satt.{{$av->productAttribute->name}}">
                                    @foreach ($av->productAttribute->attributeValues->where('product_id',$product->id) as $pav )
                                    <li class="list-item"><input type="checkbox" class="filter-link " href="#">{{ $pav->value }} </input></li>
                                    @endforeach
                                </div>
                                       </div>
                                            </ul>
