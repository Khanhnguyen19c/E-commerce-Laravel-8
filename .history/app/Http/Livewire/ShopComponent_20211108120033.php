...
class ShopComponent extends Component
{
    ...
    public function render()
    {
        ...
        $product_attribute = ProductAttribute::where('name','=','Color')->join('attribute_values','attribute_values.product_attribute_id','=','product_attributes.id')->count->get();
        $brands = Brand::all();

        $new_product_banner = HomeSlider::where('status',1)->where('type',0)->orderBy('created_at','DESC')->first();
        return view('livewire.shop-component',['product_attribute'=>$product_attribute,'brands'=>$brands,'new_product_banner'=>$new_product_banner,'products' => $products,'categories'=> $categories,'sale' => $sale,'popular_products'=>$popular_products])->layout("layouts.base");
    }
}
