...
class TopProductOnWeekComponent extends Component
{
    ...
    public function render()
    {
        ...
        if($this->sorting =='date'){
            $products = Products::whereBetween('regular_price',[$this->min_price,$this->max_price])->whereBetween('created_at',[$to,$to])->orderBy('created_at','DESC')->paginate($this->pagesize);
        }
        else if($this->sorting =='price'){
            $products = Products::whereBetween('regular_price',[$this->min_price,$this->max_price])->whereBetween('created_at',[$to,$to])->orderBy('regular_price','ASC')->paginate($this->pagesize);
        }
        else if($this->sorting =='price-desc'){
            $products = Products::whereBetween('regular_price',[$this->min_price,$this->max_price])->whereBetween('created_at',[$to,$to])->orderBy('regular_price','DESC')->paginate($this->pagesize);
        }
        else{
            $products = Products::whereBetween('regular_price',[$this->min_price,$this->max_price])->whereBetween('created_at',[$to,$to])->orderBy('created_at','DESC')->paginate($this->pagesize);
        }
